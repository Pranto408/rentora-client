...
export default function RegisterPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      // 1️⃣  Create the user via BetterAuth
  ...
  async function handleGoogle() {
    ...
    setServerError("");
    try {
      // Role is set to "tenant" by default in the OAuth callback handler
      // See: src/app/api/auth/[...all]/route.js  → onUserCreate hook
      await signIn.social({
        provider: "google",
        callbackURL: "/", // redirect after Google OAuth
      });
    } catch (err) {
      ...
      <motion.div initial="hidden" animate="show" className="w-full max-w-md">
        ...
        >
          <motion.div variants={fadeUp} custom={5}>
            <Field icon={HiOutlineUser} label="Full Name" error={errors.name}>
              <input
                name="name"
                type="text"
          ...
          <motion.div variants={fadeUp} custom={8}>
            ...
            {form.password && (
              <p
                className={`mt-2 pl-1 text-xs flex items-center gap-1.5 ${form.password.length >= 8 ? "text-green-500" : "text-[#1B3B5A]/40 dark:text-white/30"}`}
              >
